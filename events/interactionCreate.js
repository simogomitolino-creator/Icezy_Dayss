  ...
  async execute(interaction) {
    try {
      ...
      if (interaction.isButton()) {
        ...
        if (id === 'rv_submit') return reviewSystem.handleSubmit(interaction);
        if (id === 'rv_cancel') return reviewSystem.handleCancel(interaction);
      ...
      if (interaction.isModalSubmit()) {
        ...
        if (id.startsWith('rv_modal_')) {
          ...
          const [rating, guildId, product] = rest.split('_');
          return reviewSystem.handleReviewModalSubmit(interaction, rating, guildId, product);
        }
        return;
      }
    } catch (err) {
