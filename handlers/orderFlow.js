...
const { emojiForComponent, emojiForText } = require('../utils/emoji');
...
function stepsFor(product) {
  switch (product) {
    // Ranked now also asks for Power 11 brawler count, right after picking ranks.
    case 'ranked': return ['service', 'currentrank', 'desiredrank', 'power11', 'payment', 'notes', 'confirm'];
...
function render(session, guild) {
  ...
  if (step === 'payment') {
    ...
    const options = config.PAYMENT_METHODS.map((m) => ({ label: m.label, value: m.value, emoji: emojiForComponent(guild, m) }));
  ...
  if (step === 'confirm') {
    ...
    if (session.product === 'ranked') {
      ...
      const p11 = config.POWER11_OPTIONS.find((o) => o.value === session.power11);
      fields.push({ name: 'Power 11 Brawlers ⓫', value: p11 ? p11.label : 'N/A', inline: true });
    ...
    fields.push({ name: 'Payment Method 💳', value: payMeta ? `${emojiForText(guild, payMeta)} ${payMeta.label}` : 'N/A', inline: false });
...
async function startOrder(interaction, product) {
  ...
  const { embeds, components } = render(session, interaction.guild);
...
async function handleServiceButton(interaction, type) {
  ...
  if (!session) return interaction.editReply({ content: 'Your order session expired, please start again.', embeds: [], components: [] });
...
async function handleNotesChoice(interaction, add) {
  ...
  // Opening a modal MUST be the immediate response - no defer before this.
...
async function handleChange(interaction) {
  ...
  const session = sessions.get(interaction.user.id);
  if (!session) return interaction.editReply({ content: 'Your order session expired, please start again.', embeds: [], components: [] });
  const fresh = newSession(session.product);
  sessions.set(interaction.user.id, fresh);
  const { embeds, components } = render(fresh, interaction.guild);
  await interaction.editReply({ embeds, components });
}
...
async function handleConfirm(interaction) {
  ...
  try {
    ...
    console.error('⚠️ Order save failed (ticket still created):', e.message);
  }
